...
class Event(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)  # creator/owner
    ...
    def remaining_tickets(self) -> int:
        """Non-negative remaining tickets based on totals and sold count."""
        return max(0, (self.total_tickets or 0) - (self.tickets_sold or 0))

    def can_book(self, qty: int) -> bool:
        """True if event is open and has enough tickets for qty."""
        if self.status != "Open":
            return False
        if not qty or qty <= 0:
            return False
    ...
    def apply_booking(self, qty: int) -> None:
        """Increase sold count and flip to Sold Out if no tickets remain."""
    ...
    def refresh_status(self) -> None:
        """
        Recompute derived 'status' from ticket numbers unless explicitly
        set to 'Cancelled' or 'Inactive'.
        """
        lowered = (self.status or "").lower()
        if lowered in {"cancelled", "inactive"}:
            return
        self.status = "Sold Out" if self.remaining_tickets() <= 0 else "Open"

    @property
    def venue_text(self) -> str:
        """Convenient display string for venue."""
        if self.venue:
