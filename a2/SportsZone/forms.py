...
class EventForm(FlaskForm):
    ...
    sport_type = SelectField(
        ...
        validators=[
            ...
            AnyOf(
                values=["football", "basketball", "rugby", "cricket", "tennis", "hockey"],
    ...
    image = FileField(
        "Event Image",
        validators=[FileAllowed(ALLOWED_FILE, message="Only supports PNG, JPG, JPEG")],
    )
    description = TextAreaField(
        "Event description", validators=[Optional(), Length(max=5000)]
    )
    ...
    def validate_start_datetime(self, field):
        ...
        now = datetime.now()
        if field.data and field.data <= now + timedelta(minutes=1):
